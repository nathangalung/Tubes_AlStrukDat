...
/* Mengembalikan IDX_UNDEF jika tidak ditemukan */
{
    ...
    Address p = FIRST(l);
    while (p != NULL) {
        if (IsStringsEqual(INFO(p).TabWord, val.TabWord)) {
            return i;
        }
        p = NEXT(p);
        i++;
...
void displayList(List l) {
    ...
    Address p = FIRST(l);
    while (p != NULL) {
        // Menggunakan loop untuk menampilkan isi TabWord
        for (int i = 0; i < INFO(p).Length; i++) {
            printf("%c", INFO(p).TabWord[i]);
        }
        p = NEXT(p);
        if (p != NULL) {
            printf(",");
...
/* traversal ulang jika setelah Search akan dilakukan operasi lain */
{
    ...
    while (p != NULL && !IsStringsEqual(INFO(p).TabWord, X.TabWord)) {
        previous = p;
        p = NEXT(p);
    }
